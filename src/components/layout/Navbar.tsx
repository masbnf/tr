    ...
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 ${open ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}>
        <div className="backdrop-blur-md border-t border-white/10 px-6 py-4 flex flex-col gap-1" style={{ background: "rgba(13,16,53,0.98)" }}>
          {NAV_LINKS.map((l) =>
            l.tel ? (
              <a key={l.label} href={l.href} className="text-white/80 hover:text-white font-medium text-sm py-2.5 border-b border-white/10 last:border-0" onClick={() => setOpen(false)}>
                {l.label}
              </a>
            ) : (
              <Link key={l.label} href={l.href} className="text-white/80 hover:text-white font-medium text-sm py-2.5 border-b border-white/10 last:border-0" onClick={() => setOpen(false)}>
                {l.label}
              </Link>
            )
          )}
